...
__device__ inline float3 BarycentricCoords3Forward(
  ...
  const float maxd = fmaxf(fmaxf(fmaxf(fmaxf(d00, d11), d11), d20), d21);  //BUG: numerical safe...

  const float denom = d00 / maxd * d11 - d01 / maxd * d01 + kEpsilon;
  const float w1 = (d11 / maxd * d20 - d01 / maxd * d21) / denom;
  const float w2 = (d00 / maxd * d21 - d01 / maxd * d20) / denom;
...
PointTriangle3DistanceCoordsForward(
  ...
  // check if point projected to triangle
  float3 bary = BarycentricCoords3Forward(p0, v0, v1, v2);  // barycentric coordinates
  bool x_in = (0.0f <= bary.x) && (bary.x <= 1.0f);
  bool y_in = (0.0f <= bary.y) && (bary.y <= 1.0f);
  bool z_in = (0.0f <= bary.z) && (bary.z <= 1.0f);
  bool is_inside = x_in && y_in && z_in;
  ...
  if ((is_inside) && (norm_normal > kEpsilon)) {
    ...
    const float2 e01 = PointLine3DistanceCoordsForward(p, v0, v1);  // 1-tt, tt, 0
    const float2 e02 = PointLine3DistanceCoordsForward(p, v0, v2);  // 1-tt, 0, tt
    const float2 e12 = PointLine3DistanceCoordsForward(p, v1, v2);  // 0, 1-tt, tt

    if (e01.x > e02.x) {
      if (e02.x > e12.x) {
        dist = e12.x; tt = e12.y;
        bary.x = 0; bary.y = 1-tt; bary.z = tt;
        type = 1;
      } else {
        dist = e02.x; tt = e02.y;
        bary.x = 1-tt; bary.y = 0; bary.z = tt;
        type = 2;
      }
    } else {
      if (e01.x > e12.x) {
        ...
        dist = e01.x; tt = e01.y;
        bary.x = 1-tt; bary.y = tt; bary.z = 0;
        type = 3;
      }
    }
  }
