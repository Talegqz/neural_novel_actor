...
std::tuple< at::Tensor, at::Tensor, at::Tensor > cloud_intersect(
  at::Tensor ray_start, at::Tensor ray_dir, at::Tensor points, 
  const float radius){
  ...
  at::Tensor idx =
      torch::zeros({ray_start.size(0), ray_start.size(1)},
  ...
  at::Tensor max_depth =
      torch::zeros({ray_start.size(0), ray_start.size(1)},
                    at::device(ray_start.device()).dtype(at::ScalarType::Float));
  cloud_intersect_point_kernel_wrapper(points.size(0), points.size(1), ray_start.size(1),
                                      radius,
