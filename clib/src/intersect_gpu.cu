...
__global__ void cloud_intersect_point_kernel(
  ...
  idx += batch_index * m;
  min_depth += batch_index * m;
  max_depth += batch_index * m;
  ...
  for (int j = index; j < m; j += stride) {
    ...
    for (int k = 0; k < n; ++k) {
      ...
      if (r2 < radius2) {
        ...
        if ((depth - depth_blur) < _min_depth) {
          _min_depth = depth - depth_blur;
        }
        if ((depth + depth_blur) > _max_depth) {
          _max_depth = depth + depth_blur;
        }

      }
    }
  
    min_depth[j] = _min_depth;
    max_depth[j] = _max_depth;
...
void cloud_intersect_point_kernel_wrapper(
  int b, int n, int m, float radius,
  const float *ray_start, const float *ray_dir, const float *points,
  int *idx, float *min_depth, float *max_depth) {
  
  cudaStream_t stream = at::cuda::getCurrentCUDAStream();
  cloud_intersect_point_kernel<<<b, opt_n_threads(m), 0, stream>>>(
      b, n, m, radius, ray_start, ray_dir, points, idx, min_depth, max_depth);
